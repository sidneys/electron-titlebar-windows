...
class TitleBar extends EventEmitter {
     ...
     * @param {Array=} options.exclude - Exclude icons based on classname. ['minimize', 'resize', 'close']
     */
    constructor(options = {}) {
        super();
        ...
        this.options.exclude = options.exclude;
    ...
    appendTo(context = document.body) {
        ...
        if (Array.isArray(this.options.exclude)) {
            this.options.exclude.forEach((className) => {
                const el = this[`${className}Button`];
                if (el) {
                    el.remove();
                }
            });
